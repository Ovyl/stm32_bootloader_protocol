...
bool stm32_bl_erase_chip(void) {
    // Issue command
    if (!stm32_bl_write_command(STM32_BL_CMD_ERASE))
        return false;

    uint8_t erase_chip_command[] = {0xFF, 0xFF, 0x00, 0x00};
    if (!inst.write_func(erase_chip_command, sizeof(erase_chip_command)));
